-- A self-service transport payment is already the customer's payment. Once
-- Cyberpac confirms it, finalize the selected route and issue the invoice in
-- the same database transaction. The admin-created invoice flow remains
-- independent and continues to use invoice_payments.
