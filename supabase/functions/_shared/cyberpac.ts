...
const toBigInt = (bytes: Uint8Array) => bytes.reduce((value, byte) => (value << 8n) | BigInt(byte), 0n)
const toBytes = (value: bigint, length: number) => Uint8Array.from({ length }, (_, index) => Number((value >> BigInt((length - index - 1) * 8)) & 255n))
const permute = (value: bigint, table: number[], bits: number) => table.reduce((out, position) => (out << 1n) | ((value >> BigInt(bits - position)) & 1n), 0n)
const rotate28 = (value: bigint, count: number) => ((value << BigInt(count)) | (value >> BigInt(28 - count))) & 0xfffffffn
...
function deriveKey(order: string, secret: string) {
  ...
  if (source.length !== 16 && source.length !== 24) throw new Error('La clave de firma Cyberpac debe tener 16 o 24 bytes en Base64.')
  ...
  for (let offset = 0; offset < padded.length; offset += 8) {
    const encrypted = des(des(des(toBigInt(padded.slice(offset, offset + 8)) ^ previous, key.slice(0, 8), true), key.slice(8, 16), false), key.slice(16, 24), true)
...
export async function cyberpacSignature(order: string, parameters: string, secret: string) {
  const signingKey = await crypto.subtle.importKey('raw', deriveKey(order, secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign'])
  return encodeBase64Url(new Uint8Array(await crypto.subtle.sign('HMAC', signingKey, encoder.encode(parameters))))
}
