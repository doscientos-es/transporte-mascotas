...
export async function paymentUrl(invoiceId: string) {
  const existingResponse = await rest(`invoice_payments?invoice_id=eq.${encodeURIComponent(invoiceId)}&status=eq.pendiente&expires_at=gt.${encodeURIComponent(new Date().toISOString())}&select=public_token&order=created_at.desc&limit=1`)
...
export async function dispatchBillingNotifications(invoiceId: string, kind: Notification['kind'], paymentLink?: string) {
  const claimResponse = await rest('rpc/claim_billing_notifications', {
    method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ p_invoice_draft_id: invoiceId, p_kind: kind }),
  })
  ...
  for (const notification of notifications) {
    try {
      const link = kind === 'solicitud_pago' ? paymentLink : await invoiceUrl(notification.issued_invoice_id)
      if (!link) throw new Error('No se ha encontrado la factura emitida.')
      const messageId = notification.channel === 'email'
        ? await sendEmail(notification.recipient, kind, link)
        : await sendWhatsApp(notification.recipient, kind, link)
      await updateNotification(notification.id, { status: 'enviada', provider_message_id: messageId, sent_at: new Date().toISOString(), processing_started_at: null })
      sent++
    } catch (error) {
      await updateNotification(notification.id, { status: 'fallida', error_message: safeMessage(error), processing_started_at: null })
      failed++
    }
  }
  return { sent, failed }
...
async function sendEmail(recipient: string, kind: Notification['kind'], link: string) {
  ...
  const subject = kind === 'solicitud_pago' ? 'Solicitud de pago por Bizum' : 'Tu factura ya está disponible'
  const label = kind === 'solicitud_pago' ? 'Pagar de forma segura con Bizum' : 'Ver factura'
  const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [recipient], subject, html: `<p>${kind === 'solicitud_pago' ? 'Puedes abonar el servicio de forma segura.' : 'El pago se ha confirmado y tu factura está disponible.'}</p><p><a href="${link}">${label}</a></p>`, text: `${label}: ${link}` }) })
...
async function sendWhatsApp(recipient: string, kind: Notification['kind'], link: string) {
  ...
  const template = Deno.env.get(kind === 'solicitud_pago' ? 'META_WHATSAPP_PAYMENT_TEMPLATE' : 'META_WHATSAPP_INVOICE_TEMPLATE')
  if (!token || !phoneNumberId || !template) throw new Error('WhatsApp Business todavía no está configurado.')
  ...
  const response = await fetch(`https://graph.facebook.com/${version}/${phoneNumberId}/messages`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ messaging_product: 'whatsapp', to: normalizePhone(recipient), type: 'template', template: { name: template, language: { code: 'es' }, components: [{ type: 'body', parameters: [{ type: 'text', text: firstValue }, { type: 'text', text: link }] }] } }) })
...
function normalizePhone(value: string) {
  ...
  if (!/^[1-9][0-9]{7,14}$/.test(digits)) throw new Error('El teléfono de WhatsApp debe estar en formato internacional.')
  return digits
}

async function updateNotification(id: string, body: Record<string, unknown>) {
  await rest(`billing_notifications?id=eq.${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(body) })
}
