...
type Invoice = { id: string; status: string; total_amount: string; client_snapshot: Record<string, unknown> }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    return json({ error: error instanceof Error ? error.message : 'No se ha podido registrar el cobro.' }, 500)
  }
})

async function manualPayment(invoice: Invoice) {
  const existingResponse = await rest(`invoice_payments?invoice_id=eq.${encodeURIComponent(invoice.id)}&provider=eq.manual&select=id&limit=1`)
  ...
  const response = await rest('invoice_payments', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ invoice_id: invoice.id, provider: 'manual', merchant_order: `M${crypto.randomUUID().replaceAll('-', '').slice(0, 11)}`, amount_cents: amountCents }) })
...
function validateFiscalClient(client: Record<string, unknown>) {
  ...
  if (required.some((field) => typeof client[field] !== 'string' || !client[field].trim())) throw new Error('Completa los datos fiscales antes de registrar el cobro.')
}

function issuerSnapshot() {
  const name = Deno.env.get('INVOICE_ISSUER_NAME'); const taxId = Deno.env.get('INVOICE_ISSUER_TAX_ID'); const address = Deno.env.get('INVOICE_ISSUER_ADDRESS')
