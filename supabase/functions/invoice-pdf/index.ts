...
async function signedToken(invoiceId: string, userId: string) {
  const payload = base64url(
    encoder.encode(JSON.stringify({ invoiceId, userId, expiresAt: Date.now() + 10 * 60_000 })),
  )
...
async function verifyToken(token: string) {
  ...
  if (!payload || !receivedSignature || receivedSignature !== (await signature(payload)))
    throw new Error('El enlace de la factura no es válido.')
  const value = JSON.parse(decoder.decode(fromBase64url(payload))) as {
    invoiceId?: string
    expiresAt?: number
  }
  if (!value.invoiceId || !value.expiresAt || value.expiresAt < Date.now())
    throw new Error('El enlace de la factura ha caducado.')
...
Deno.serve(async (request) => {
  try {
    ...
    if (request.method === 'POST') {
      ...
      return json({
        url: `${functionUrl}?token=${encodeURIComponent(token)}`,
    ...
    return json(
      { error: error instanceof Error ? error.message : 'No se ha podido generar la factura.' },
      500,
    )
  }
})
