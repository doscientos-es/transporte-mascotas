...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const invoiceResponse = await rest(
      `invoice_drafts?id=eq.${encodeURIComponent(invoiceId)}&select=id,total_amount,status,client_snapshot`,
    )
    ...
    if (invoice.status !== 'solicitud_pago')
      return json({ error: 'Esta solicitud ya no admite pagos.' }, 409)
    ...
    return json(
      { error: error instanceof Error ? error.message : 'No se ha podido crear el pago.' },
      500,
    )
  }
})
...
async function createPayment(invoice: Invoice) {
  ...
  if (!Number.isSafeInteger(amountCents) || amountCents <= 0)
    throw new Error('El importe de la factura no es válido.')
