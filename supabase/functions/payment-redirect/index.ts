...
type Payment = { merchant_order: string; amount_cents: number; status: string; expires_at: string; invoice_id: string }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const paymentResponse = await rest(`invoice_payments?public_token=eq.${encodeURIComponent(token)}&select=merchant_order,amount_cents,status,expires_at,invoice_id`)
    ...
    return new Response(form(config.endpoint, parameters, signature), { headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' } })
  } catch (error) {
...
function configuration() {
  ...
  if (!merchantCode || !terminal || !secret || !publicAppUrl || !endpoint || !supabaseUrl) throw new Error('Bizum para comercios de CaixaBank todavía no está configurado.')
...
function page(message: string, status: number) {
  return new Response(`<!doctype html><html lang="es"><body><h1>${message}</h1><p>Contacta con el comercio si necesitas ayuda.</p></body></html>`, { status, headers: { 'Content-Type': 'text/html; charset=utf-8' } })
}
