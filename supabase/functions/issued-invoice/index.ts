...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const response = await rest(`issued_invoices?public_token=eq.${encodeURIComponent(token)}&select=id,invoice_draft_id,document_expires_at`)
    ...
    if (!document || document.issued_invoice_id !== invoice.id) return page('La factura todavía se está preparando.', 503)
    const storedFile = await fetchInvoiceDocument(document)
    return new Response(storedFile.body, { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': `inline; filename="${document.file_name}"; filename*=UTF-8''${encodeURIComponent(document.file_name)}`, 'Cache-Control': 'private, no-store', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer' } })
  } catch {
