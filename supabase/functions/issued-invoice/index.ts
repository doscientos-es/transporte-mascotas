...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const logoUrl = appUrl ? new URL('/logo-light.svg', appUrl).toString() : null
    const imageSource = logoUrl ? new URL(logoUrl).origin : "'none'"
    return new Response(document(invoice, logoUrl), { headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer', 'Content-Security-Policy': `default-src 'none'; style-src 'unsafe-inline'; img-src ${imageSource}` } })
  } catch {
    return page('No se ha podido abrir la factura.', 503)
  }
})

function document(invoice: Invoice, logoUrl: string | null) {
