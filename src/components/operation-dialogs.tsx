...
export function InvoiceDialog({ letter, onClose, onGenerate }: { letter: Letter; onClose: () => void; onGenerate: (letter: Letter, payer: 'remitente' | 'destinatario', total: number) => Promise<void> }) {
  const [payer, setPayer] = useState<'remitente' | 'destinatario'>('remitente')
  const [total, setTotal] = useState('200')
  const [generating, setGenerating] = useState(false)
  const generate = () => {
    setGenerating(true)
    onGenerate(letter, payer, Number(total) || 0).finally(() => {
      setGenerating(false)
      onClose()
    })
  }
