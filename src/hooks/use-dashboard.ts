...
import { loadLetters, saveManualLetter, updateLetter } from '../lib/letters'
import {
  addDailyRouteStop,
...
import { assignmentsForRoute, boxesBySize } from '../lib/van'
...
function actionsForLetter(route: DailyRoute, template: RouteTemplate, letter: Letter) {
  ...
  const originStop = stops.find(
    (stop) => stop.locality.trim().toLocaleLowerCase() === letter.origin.trim().toLocaleLowerCase(),
  )
  const destinationStop = stops.find(
    (stop) =>
      stop.locality.trim().toLocaleLowerCase() === letter.destination.trim().toLocaleLowerCase(),
  )
...
export function useDashboard(session: Session | null, role: AppRole) {
  ...
  async function updateActions(actionIds: string[]) {
    if (!selectedRoute || !session)
      return toast('Inicia sesión para actualizar las acciones de ruta.')
    ...
    const results = await Promise.all(
      targets.map((action) =>
        requireSupabase().rpc('record_route_action', { p_action_id: action.id, p_status: status }),
      ),
    )
    if (results.some(({ error }) => error))
      return toast('No se han podido actualizar las acciones en la ruta.')
