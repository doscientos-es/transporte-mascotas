...
export function RoutesPage({ route, template, templates, routes, letters, onSelect, onAction, onUpdateStops, onCreate, canManage = true }: Props) {
  const pageSize = 12
  ...
  useEffect(() => { setOrganizing(false) }, [route.id])

  function moveStop(index: number, direction: -1 | 1) { const target = index + direction; if (target < 0 || target >= stops.length) return; const next = [...stops]; [next[index], next[target]] = [next[target], next[index]]; onUpdateStops(route.id, next) }
  function setDwellMinutes(index: number, value: string) { const dwellMinutes = Math.max(0, Number(value) || 0); onUpdateStops(route.id, stops.map((stop, stopIndex) => stopIndex === index ? { ...stop, dwellMinutes } : stop)) }

  return <><PageIntro text={canManage ? `Organiza las paradas y sus tiempos · ${directionLabel(direction)}.` : `Consulta el itinerario asignado · ${directionLabel(direction)}.`}>{canManage && <Button onClick={onCreate}><CalendarDays /> Crear ruta</Button>}</PageIntro><div className="route-selector">{visibleRoutes.map((item) => <button type="button" onClick={() => onSelect(item)} className={route.id === item.id ? 'is-selected' : ''} key={item.id}><Route size={16} /><span>{templates.find((current) => current.id === item.templateId)?.name}<small>{directionLabel(item.direction ?? 'normal')}</small></span><b>{new Date(`${item.date}T12:00:00`).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}</b><span className={`status status-${item.status}`}>{statusLabels[item.status]}</span></button>)}</div><Pagination page={page} pageCount={pageCount} firstRecord={routes.length === 0 ? 0 : (page - 1) * pageSize + 1} lastRecord={Math.min(page * pageSize, routes.length)} total={routes.length} ariaLabel="Paginación de rutas" onChange={setPage} /><Card className="route-journey"><CardContent><div className="journey-header"><div><span className="eyebrow">{route.date}</span><h3>Ruta {template.name}</h3><span className={`route-direction-badge direction-${direction}`}>{directionLabel(direction)}</span></div><div className="journey-actions"><div className="route-total"><Clock3 size={16} /><span>Estimación total</span><strong>{formatDuration(travelMinutes + pointMinutes)}</strong><small>{formatDuration(travelMinutes)} trayectos · {formatDuration(pointMinutes)} paradas</small></div><span className={`status status-${route.status}`}>{statusLabels[route.status]}</span>{canManage && <Button variant="outline" size="sm" onClick={() => setOrganizing((current) => !current)}>{organizing ? 'Terminar' : 'Organizar paradas'}</Button>}</div></div>{organizing && <div className="itinerary-toolbar"><span><Clock3 size={15} /> Salida estimada a las 08:00. Usa las flechas y ajusta los minutos de espera.</span></div>}<ol>{stops.map((stop, index) => <JourneyStop key={stop.id} stop={stop} index={index} total={stops.length} arrival={formatArrival(route.date, arrivalByStop.get(stop.id) ?? 0)} organizing={organizing} services={servicesByStop.get(stop.id) ?? []} onMove={moveStop} onDwellChange={setDwellMinutes} onAction={onAction} />)}</ol></CardContent></Card></>
}

function JourneyStop({ stop, index, total, arrival, organizing, services, onMove, onDwellChange, onAction }: { stop: DailyRouteStop; index: number; total: number; arrival: string; organizing: boolean; services: ServiceGroup[]; onMove: (index: number, direction: -1 | 1) => void; onDwellChange: (index: number, value: string) => void; onAction: (ids: string[]) => void }) {
