...
export function RoutesPage({ route, template, templates, routes, onSelect, onAction, onUpdateStops, onUpdateService, onRemoveService, onCreate }: Props) {
  ...
  const serviceMinutes = [...servicesByStop.values()].flat().reduce((total, action) => total + (action.dwellMinutes ?? 15), 0)
  const totalMinutes = travelMinutes + pointMinutes + serviceMinutes
  const elapsedByStop = new Map<string, number>()
  let elapsedMinutes = 0
  stops.forEach((stop, index) => {
    elapsedMinutes += stop.dwellMinutes + (servicesByStop.get(stop.id) ?? []).reduce((total, action) => total + (action.dwellMinutes ?? 15), 0)
