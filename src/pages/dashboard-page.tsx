...
const AdminDashboardPage = lazy(() => import('./admin-dashboard-page').then(({ AdminDashboardPage: page }) => ({ default: page })))
const ClientPortalPage = lazy(() => import('./client-portal-page').then(({ ClientPortalPage: page }) => ({ default: page })))

export function DashboardPage({ session, profile }: { session: Session | null; profile: UserProfile }) {
  return <Suspense fallback={<div className="loading-screen">Cargando…</div>}>
