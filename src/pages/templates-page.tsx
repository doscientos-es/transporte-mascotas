...
import {
  ...
  StopFormDialog,
  type StopFormValues,
} from '../components/operation-dialogs'
...
export function TemplatesPage({
  ...
  function dropIndexForStop(event: DragEvent<HTMLLIElement>, index: number) {
    const { top, height } = event.currentTarget.getBoundingClientRect()
    return event.clientY < top + height / 2 ? index : index + 1
  }

  function dragOverStop(event: DragEvent<HTMLLIElement>, index: number) {
    event.preventDefault()
      ...
      <div className="template-layout">
        ...
        <Card className="stops-card">
          <CardContent>
            ...
            {movingStopId && (
              <p className="template-move-notice" role="status">
